...
void do_app_draw() {
	...
	gfx_puts(WINDOW_EXT(window)->context, 10, 20, 0, WIN_BACKGROUND_COLOR, buff);

	window_send_message_simple(window, WINDOW_LIB_MESSAGE_PREDRAW);
}
...
int main() {
	...
	while(window_get_message(window, &msg)) { 
		...
		window_dispatch_message(window, &msg);
	}
	return 0;
}